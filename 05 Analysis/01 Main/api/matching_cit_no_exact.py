...
def load_data(citations_path = "/content/drive/MyDrive/PhD Data/08 Citations/03 Patent citations - raw, filing.pickle", 
    ...
    if 'patents_metadata' not in locals():
        patents_metadata = pd.read_stata(all_patents_metadata)
        patents_metadata['patent_id'] = patents_metadata['patent_id'].astype(str)
    
    # Merge metadata into control
    control = control.merge(
        patents_metadata[['patent_id', 'patent_date']],
        on='patent_id',
        how='left'
    )
...
def compute_cosine_distances(treated, control):
    ...
    # Save to file
    with open("/content/drive/MyDrive/PhD Data/11 Matches/actual results/citation_no_exact_match_on_grantyear/_aux/cosine_distance_by_treated.pkl", "wb") as f:
        pickle.dump(cosine_distance_by_treated, f)

    with open("/content/drive/MyDrive/PhD Data/11 Matches/actual results/citation_no_exact_match_on_grantyear/_aux/group_to_candidate_ids.pkl", "wb") as f:
...
"""def compute_cosine_distances(treated, control, batch_size=25000, device='cuda'):
    ...
    save_path = "/content/drive/MyDrive/PhD Data/11 Matches/actual results/citation_no_exact_match_on_grantyear/_aux/cosine_distance_by_treated.pkl"
    cosine_distance_by_treated = {}
...
def _save_mahalanobis(treated_sample, control, citation_counts_dict, treated_counts_dict,
    ...
    path = f"/content/drive/MyDrive/PhD Data/11 Matches/actual results/citation_no_exact_match_on_grantyear/_aux/{filename}"
...
def prepare(
    ...
    if not os.path.exists("/content/drive/MyDrive/PhD Data/11 Matches/actual results/citation_no_exact_match_on_grantyear/_aux/control.pkl"):
    ...
    for baseline_begin_period in baseline_periods:
        ...
        with open(f"/content/drive/MyDrive/PhD Data/11 Matches/actual results/citation_no_exact_match_on_grantyear/_aux/treated_counts_dict_{suffix}.pkl", "wb") as f:
...
def prepare_sample(treated, acq_type, treated_counts_dict,  top_tech = False, top_tech_threshold=80, baseline_begin_period = 8):

    """Prepare treated sample based on acquisition type and top tech status."""
    import os 

    # If the file exists, just load it
    treated_path = f"/content/drive/MyDrive/PhD Data/11 Matches/actual results/citation_no_exact_match_on_grantyear/_aux/treated_{acq_type}_top_tech_{top_tech_threshold}_{baseline_begin_period}q.pkl" if top_tech else f"/content/drive/MyDrive/PhD Data/11 Matches/actual results/citation_no_exact_match_on_grantyear/_aux/treated_{acq_type}_bl.pkl"
    ...
    if top_tech:
        treated_sample.to_pickle(f"/content/drive/MyDrive/PhD Data/11 Matches/actual results/citation_no_exact_match_on_grantyear/_aux/treated_{acq_type}_top_tech_{top_tech_threshold}_{baseline_begin_period}q.pkl")
    else:
        treated_sample.to_pickle(f"/content/drive/MyDrive/PhD Data/11 Matches/actual results/citation_no_exact_match_on_grantyear/_aux/treated_{acq_type}_bl.pkl")
...
def load_aux_data(acq_type, top_tech=False, top_tech_threshold=90, baseline_begin_period=13):
    ...
    if 'citation_counts_dict' not in locals():
        with open(f"/content/drive/MyDrive/PhD Data/11 Matches/actual results/citation_no_exact_match_on_grantyear/_aux/citation_counts_dict.pkl", "rb") as f:
            citation_counts_dict = pickle.load(f)

    if 'cosine_distance_by_treated' not in locals():
        with open(f"/content/drive/MyDrive/PhD Data/11 Matches/actual results/citation_no_exact_match_on_grantyear/_aux/cosine_distance_by_treated.pkl", "rb") as f:
            cosine_distance_by_treated = pickle.load(f)
    
    if 'control' not in locals():
        control = pd.read_pickle("/content/drive/MyDrive/PhD Data/11 Matches/actual results/citation_no_exact_match_on_grantyear/_aux/control.pkl")

    # Load treated counts dictionary anyways
    with open(f"/content/drive/MyDrive/PhD Data/11 Matches/actual results/citation_no_exact_match_on_grantyear/_aux/treated_counts_dict_{suffix}.pkl", "rb") as f:
        treated_counts_dict = pickle.load(f)

    # Load treated sample
    if top_tech:
        treated = pd.read_pickle(
            f"/content/drive/MyDrive/PhD Data/11 Matches/actual results/citation_no_exact_match_on_grantyear/_aux/treated_{acq_type}_top_tech_{top_tech_threshold}_{baseline_begin_period}q.pkl"
        )
    else:
        treated = pd.read_pickle(
            f"/content/drive/MyDrive/PhD Data/11 Matches/actual results/citation_no_exact_match_on_grantyear/_aux/treated_{acq_type}_bl.pkl"
    ...
    if 'group_to_candidate_ids' not in locals():
        with open("/content/drive/MyDrive/PhD Data/11 Matches/actual results/citation_no_exact_match_on_grantyear/_aux/group_to_candidate_ids.pkl", "rb") as f:
...
def run_routine(treated, control, citation_counts_dict, treated_counts_dict, cosine_distance_by_treated, group_to_candidate_ids,
                caliper=0.05, lambda_start=0, lambda_end=1, delta=0.25, baseline_begin_period=8, precomputed_mahalanobis=None, K = 10):
    """
