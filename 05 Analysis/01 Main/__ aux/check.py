        ...
        for caliper in calipers:
            for lambda_value in lambdas:
              for outcome_var in outcome_variables:
                  ...
                  """
                  if lambda_value == 1:
                      xformla = "~ C(grant_year_f) + C(cpc_f)"
                  else:
                      pc_terms = " + ".join(pca_cols)
                      xformla = f"~ grant_year_code + cpc_code + {pc_terms}"
                      #xformla = "~ " + " + ".join(covar_cols)
                  """
...
run_log_path = log_dir / "08a_estimate_python_run_log_Off Deal_bl_nearby.csv"
