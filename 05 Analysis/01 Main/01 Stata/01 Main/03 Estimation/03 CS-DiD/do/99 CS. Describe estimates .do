...
* ==============================================================================
    ...
    ** Start log
    ...
    log using "${log}/99a CS. Describe estimates -`ymd'_`hhmmss'.log", replace
    timer clear 1
    timer on 1
    ...
    ** Loop over ster files
    foreach f of local ster_files {
        ...
        ** Parse file name to get parameters
        ...
        local filename = subinstr("`filename'", "10. CSDID Estimates - ", "", .)
        local filename = subinstr("`filename'", "11. CSDID Estimates - ", "", .)

        local filename = subinstr("`filename'", "- all patents, for csdid - ", ",", .)
        local filename = subinstr("`filename'", ".dta", "", .)

            di in red "Config: `filename'"
            ...
            preserve
                ...
                ** Extract parameters   
                if regexm(config[1], "baseline") {
                    drop config 
                    rename (config*) (acq_type bl_tt pretreatment_periods caliper lambda posttreatment_periods)
                }
                else {
                    drop config 
                    rename (config*) (acq_type bl_tt tt_threshold pretreatment_periods caliper lambda posttreatment_periods)
                }
                gen est_config = "no exact matching on grant year"
            ...
            ** Get simple ATT estimate and its SE and p-value
                ...
                replace simple = el(r(table),1,1) if _n == `i'
                replace simple_se = el(r(table),2,1) if _n == `i'
                replace simple_p = el(r(table),4,1) if _n == `i'
                ...
                ** Pretrend p-values
                ...
                forvalues t = `=-`pre' + 1' / `post' {
                    ...
                    ** Get estimates and return matrices
                    ...
                    replace es_`t' = el(r(table), 1, 3 + `counter') if _n == `i'
                    replace es_`t'_se = el(r(table), 2, 3 + `counter')  if _n == `i'
                    replace es_`t'_p = el(r(table), 4, 3 + `counter') if _n == `i'
...
awd
    ** Save final results
    save "${dta}\CSDID Estimates Summary (extended, CS estimates).dta", replace