...
* ==============================================================================
    ...
    ** Config
    ...
    local lambdas 1 // numlist(0.0(0.05)1.0) 
    ...
    ** Start log
    log using "${log}/02. Estimate - no PCA for lambda 1.log", replace
    timer clear 1
    timer on 1
            ...
            foreach caliper of local calipers {
                foreach lambda of local lambdas {
                    ...
                    ** Skip if already estimated
                    ...
                    local est_filename = subinstr("`filename'", "01 Sample", "02 CSDID Estimates", .)
                ...
                * ==============================================================================
                    ...
                    *** Find the earliest quarter where there is at least one treated patent x quarter 
                    /* sum cohort if cohort > 0, d 
                    local first_treated_quarter = r(min) 
                    local last_treated_quarter = r(max) 

                    count if treated
                    drop if (quarter < `first_treated_quarter' - `pre_treatment_period') | (quarter > `last_treated_quarter' + `last_post_treatment_period')
                    count if treated */
                ...
                * ==============================================================================
                    ...
                    ** Run csdid
                    /* local rif_name = subinstr("`est_filename'lambda`lambda', p`est_range_str'", "01 CSDID Estimates - ", "01 RIF - csdid estimates", .)
                    local rif_name = subinstr("`rif_name'", " ", "_", .)
                    local rif_name = subinstr("`rif_name'", "-", "_", .)
                    local rif_name = subinstr("`rif_name'", ",", "_", .)
                    local rif_name = subinstr("`rif_name'", ".", "_", .)
                    local rif_name = subinstr("`rif_name'", "__", "_", .)
                    local rif_name = subinstr("`rif_name'", "___", "_", .)

                    di in red "RIF name: `rif_name'" */

                    csdid log_citation age* i.grant_year i.cpc, i(patent_id) g(cohort) t(quarter) method(reg) seed(`seed') 
