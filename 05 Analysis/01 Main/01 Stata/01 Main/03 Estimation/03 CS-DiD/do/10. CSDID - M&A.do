...
* ==============================================================================
    ...
    *---------------------------------------------------------*
        * Get grant date of controls
        u "${stata}\01 Main\01 Data load\00 Patents\dta\01 Patent data - without citations - only matched records - no exact match on grant year.dta", clear
...
* ==============================================================================
    ...
    foreach pre_treatment_period of local pre_treatment_periods { 
        foreach acq_type of local acq_types {
            foreach base_tt of local sample_types{
                foreach caliper of local calipers {
                    foreach lambda of local lambdas {
                        * Filename extension depending on lambda
                        if inlist(`lambda', 0, 1){
                            local lam_ext  ""
                        }
                        else{
                            local lam_ext  " (lambda 0.6 and 0.7)"
                        }
                        ...
                        ** Get the filename - e.g. "01 Sample - M&A, baseline, 4q, caliper0.1000 - all patents, for csdid.dta" or "01 Sample - Off deal, top-tech, 80, 4q, caliper0.0250 - all patents, for csdid.dta"
                        if "`base_tt'" == "baseline" {
                            ** Adjust filename 
                            local filename = "01 Sample - `acq_type', `base_tt', `=string(`pre_treatment_period', "%2.0f")'q, caliper`caliper' - all patents, for csdid - no exact matching on grant year`lam_ext'.dta"
                        }
                        else if "`base_tt'" == "top-tech" {
                            ** Adjust filename 
                            local filename = "01 Sample - `acq_type', `base_tt', `=string(`base_tt_threshold', "%2.0f")', `=string(`pre_treatment_period', "%2.0f")'q, caliper`caliper' - all patents, for csdid - no exact matching on grant year`lam_ext'.dta"
                        }
                        else {
                        ...
                        ** Build up the estimation filename & Skip if already estimated
                        local est_range_str = " -`pre_treatment_period' - `last_post_treatment_period'" // range for plots
                        local est_filename = subinstr("`filename'", "01 Sample", "10. CSDID Estimates", .)
                        local est_filename = subinstr("`est_filename'", " - all patents, for csdid - no exact matching on grant year`lam_ext'.dta", "", .)
                    ...
                    * ==============================================================================
                        ...
                        ** Run csdid
                        ...
                        if `lambda' == 1 {
                            ...
                            csdid log_citation age* i.grant_year i.cpc active, g(cohort) t(quarter) method(reg) seed(`seed') 
                        }
                        else {
