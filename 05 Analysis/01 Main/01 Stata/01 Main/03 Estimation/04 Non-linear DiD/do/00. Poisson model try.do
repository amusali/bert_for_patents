...
* ==============================================================================
   ...
   * cd "${out}\est"
    ...
    ** Config
    ...
    local lambdas 0.7 // numlist(0.0(0.05)1.0) 
    ...
    ** Start log
   * log using "${log}/09a. Estimate - no PCA for lambda 1 & tt80 (lam 0.6 and 0.7).log", replace
    timer clear 1
    timer on 1
...
* ==============================================================================
    ...
    *---------------------------------------------------------*
        * Get grant date of controls
        ...
        u "C:\Users\amusali\Desktop\uc3m PhD\05 Analysis\01 Main\01 Stata\01 Main\01 Data load\00 Patents\dta\01 Patent data - without citations - only matched records - no exact match on grant year (lam 0.6 and 0.7 version).dta", clear
            ...
            foreach caliper of local calipers {
                foreach lambda of local lambdas {
                    ...
                    ** Get the filename - e.g. "01 Sample - M&A, baseline, 4q, caliper0.1000 - all patents, for csdid.dta" or "01 Sample - Off deal, top-tech, 80, 4q, caliper0.0250 - all patents, for csdid.dta"
                    ...
                    local est_filename = subinstr("`filename'", "01 Sample", "09a CSDID Estimates", .)
